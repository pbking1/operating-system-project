...
void replaceLRU(deque<Process>& q, Process& p){
	...
	auto it = max_element(q.begin(), q.end(), timer_compare);
...
void replaceOPT(deque<Process>& q, Process& p){
	...
	auto it = max_element(q.begin(), q.end(), next_ref_compare);
...
int OPT(int x){
	...
	for (iter = process_list.begin(); iter != process_list.end(); iter++){
		// update next_ref
		for (auto& p : q){
			updateOptimum(p, process_list, iter, x);
		}
